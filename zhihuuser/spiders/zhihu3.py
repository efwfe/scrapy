...
class ZhihuSpider(scrapy.Spider):
    ...
    followes_url = "https://www.zhihu.com/api/v4/members/{user}/followees?include={include}&offset={offset}&limit={limit}"
    followes_query = "data[*].answer_count,articles_count,gender,follower_count,is_followed,is_following,badge[?(type=best_answerer)].topics"
    ...
    def start_requests(self):
        ...
        yield Request(self.followes_url.format(user=self.start_user, include=self.followes_query, offset=0, limit=20),
                      callback=self.parse_follower)
    ...
    def parse_user(self, response):
        ...
        # 获取关注列表
        yield Request(
            self.followes_url.format(user=result.get('url_token'), include=self.follower_query, offset=0, limit=20),
            callback=self.parse_follower)
